...
--INSERT         - Column matching by position, New columns not allowed
...
--OVERWRITE      - Column matching by position, New columns not allowed
...
SELECT id, fname, lname FROM json.`abfss://dbfs-container@adlssmiloeabronzedev001.dfs.core.windows.net/people.json`

-- COMMAND ----------

MERGE INTO dev.demo_db.people_tbl tgt
USING (SELECT id, fname, lname FROM json.`abfss://dbfs-container@adlssmiloeabronzedev001.dfs.core.windows.net/people.json`) src
ON tgt.id = src.id
...
MERGE INTO dev.demo_db.people_tbl tgt
USING (SELECT id, fname as firstName, lname as lastName FROM json.`abfss://dbfs-container@adlssmiloeabronzedev001.dfs.core.windows.net/people_2.json`) src
ON tgt.id = src.id
WHEN NOT MATCHED THEN
    INSERT *  
...
MERGE INTO dev.demo_db.people_tbl tgt
USING (SELECT id, fname firstName, lname lastName, dob FROM json.`abfss://dbfs-container@adlssmiloeabronzedev001.dfs.core.windows.net/people_3.json`) src
ON tgt.id = src.id
WHEN NOT MATCHED THEN
    INSERT *
...
-- MAGIC people_df =  spark.read.format("json").schema(people_schema).load("abfss://dbfs-container@adlssmiloeabronzedev001.dfs.core.windows.net/people_2.json")
